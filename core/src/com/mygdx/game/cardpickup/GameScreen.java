...
public class GameScreen extends BaseScreen {
	...
	public void create() {
		...
		background.setTexture( new Texture(Gdx.files.internal("cardpickup/felt.jpg")) );
		mainStage.addActor(background);

		pileList = new ArrayList<Pile>();
		Texture pileTex = new Texture(Gdx.files.internal("cardpickup/cardBack.png"));
		for (int n = 0; n < 4; n++)
		{
			Pile pile = new Pile();
		...
		for (int r = 0; r < rankNames.length; r++){
			for (int s = 0; s < suitNames.length; s++){
				...
				final Card card = new Card( rankNames[r], suitNames[s] );
				String fileName = "cardpickup/card" + suitNames[s] + rankNames[r] + ".png";
		...
		Texture glowTex = new Texture(Gdx.files.internal("cardpickup/glowBlue.png"));
